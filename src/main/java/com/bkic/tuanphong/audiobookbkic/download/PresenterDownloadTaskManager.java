package com.bkic.tuanphong.audiobookbkic.download;
            ...
            if (code == PackageManager.PERMISSION_GRANTED) {
                ...
                if(isDirectoryCreated) {
                    ...
                    apkSubStorage = new File(
                            Environment.getExternalStorageDirectory() + "/"
                                    + Utils.downloadDirectory + "/" + BookDownloadedTitle(context,BookId));
                    boolean isSubDirectoryCreated = apkSubStorage.exists();
                    if(!isSubDirectoryCreated) {
                        isSubDirectoryCreated = apkSubStorage.mkdir();
                        Log.d(TAG, "Sub Directory: " + BookDownloadedTitle(context,BookId) + " is created");
                    }
                    if(isSubDirectoryCreated){

                        //Check if file exists old file
                        String filePath = Environment.getExternalStorageDirectory()+"/"+Utils.downloadDirectory+"/"+BookDownloadedTitle(context,BookId)+"/"+ChapterDownloadedTitle(context,BookId,ChapterId)+".mp3"; //i.e. /sdcard/AudioBookBKIC/2162/2168.mp3
...
//          request.setDestinationInExternalFilesDir(MainActivityDownloadManager.this, Environment.DIRECTORY_DOWNLOADS,"AndroidTutorialPoint.mp3");
        request.setDestinationInExternalPublicDir(Utils.downloadDirectory+"/"+BookDownloadedTitle(context,BookId),ChapterDownloadedTitle(context,BookId,ChapterId)+".mp3");
